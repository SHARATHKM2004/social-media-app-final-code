...
export default function ProfilePage() {
  ...
  const {
    ...
    changePassword,
    deleteAccount,
    refreshProfile, // ✅ available from hook
  } = useProfileData();
  ...
  async function handleDeletePost(postId: string) {
    ...
    // ✅ Better than window.location.reload(): refresh profile data
    try {
      ...
      <main className="min-h-screen bg-neutral-50 pb-16">
        <div className="mx-auto max-w-md p-5">
          <div className="rounded-3xl border-2 border-brand-blue bg-white p-5 shadow-soft">
            {/* ✅ Inline Header (fixes your build error) */}
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-60"
            ...
            {loading ? (
              ...
              <>
                ...
                {actionOk ? (
                  <p className="mt-3 text-sm text-green-600">{actionOk}</p>
                ) : null}
              </>
            )}
          </div>
